...
const MegaMenu = () => {
  ...
  return (
    <div className="flex  items-center h-16 justify-center bg-gray-100">
      {Object.entries(megaMenuData).map(([mainCategory, subGroups], i) => {
        ...
        return (
          <div
            ...
            className={`${mainCategory === 'Elektronik' ? 'border-x-2 border-gray-200':''} hover:bg-white py-1 px-4`}
          >
            ...
            {activeCategory === mainCategory && (
              <div className="absolute top-full left-0 mt-[-20px] p-2 bg-white shadow-lg  w-full flex gap-20 justify-center z-50 text-center  border-b-2 border-blue-300">
                {Object.entries(subGroups).map(([groupTitle, items]) => (
                  <div key={groupTitle} className="p-2 mt-2 ">
                    ...
                    <ul className="space-y-1 text-sm text-gray-600 text-start">
                      {items.map((item) => {
                        console.log(
                          "item",
